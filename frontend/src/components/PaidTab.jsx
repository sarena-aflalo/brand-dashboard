...
function AdCard({ ad, rank }) {
  ...
  const proxyUrl = ad.thumbnail_url
  ...
  return (
    <div className="flex flex-col">
      <div
        className="relative rounded-lg overflow-hidden bg-gray-100 aspect-square mb-2"
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        <span className="absolute top-1.5 left-1.5 bg-black/50 text-white text-xs font-medium px-1.5 py-0.5 rounded z-10">
          {rank}
        </span>
        {ad.video_url ? (
          <video
            ...
            loop
            playsInline
            poster={proxyUrl ?? undefined}
          />
        ) : proxyUrl ? (
          <img src={proxyUrl} alt={ad.creative_name} className="w-full h-full object-cover" />
        ) : (
